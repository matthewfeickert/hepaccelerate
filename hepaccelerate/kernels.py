...
def cartesian_to_spherical(backend, px, py, pz, e):
    return cartesian_to_spherical(px, py, pz, e)
...
def mask_deltar_first(backend, objs1, mask1, objs2, mask2, drcut):
    backend.mask_deltar_first(objs1, mask1, objs2, mask2, drcut)

def histogram_from_vector(backend, data, weights, bins, mask=None):
    backend.histogram_from_vector(backend, data, weights, bins, mask=mask)
 
def histogram_from_vector_several(backend, variables, weights, mask):
    backend.histogram_from_vector_several(backend, variables, weights, mask) 

def get_bin_contents(backend, values, edges, contents, out):
    backend.get_bin_contents(backend, values, edges, contents, out) 

def copyto_dst_indices(backend, dst, src, inds_dst):
    backend.copyto_dst_indices(backend, dst, src, inds_dst) 

def compute_new_offsets(backend, offsets_old, mask_objects, offsets_new):
    backend.compute_new_offsets(backend, offsets_old, mask_objects, offsets_new) 
